	...
	return (
		<SectionLayout id="testimonials" className="py-16 md:py-24">
			<div className="flex flex-col items-center justify-center text-center mb-10 sm:mb-16 px-4">
				<h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3 sm:mb-4">{t("title")}</h2>
				<p className="text-base sm:text-lg text-muted-foreground max-w-2xl">{t("description")}</p>
			</div>
					...
					<div className="flex shrink-0 justify-around [gap:var(--gap)] animate-marquee flex-row group-hover:[animation-play-state:paused]">
						{[...Array(4)].map((_, setIndex) =>
							processedTestimonials.map((testimonial, i) => (
								<TestimonialCard key={crypto.randomUUID()} author={testimonial.author} text={testimonial.text} className="shrink-0" />
							)),
						)}
					</div>
				</div>
