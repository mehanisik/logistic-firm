	...
	return (
		...
		>
			...
			<div className="absolute hidden md:block inset-5 rounded-xl overflow-hidden">
				<Image src={"/images/hero-image.png"} alt={t("companyName")} fill className="object-cover object-center" priority />
			</div>
			<div className="absolute inset-0 z-10 flex items-center justify-center">
				<div className="max-w-3xl mx-auto text-center relative">
					...
					>
						<span className="text-sm font-extrabold text-black dark:text-white tracking-wide">{t("companyName")}</span>
					</motion.div>

					<motion.div custom={1} variants={fadeUpVariants} initial="hidden" animate="visible">
						<h1 className="text-6xl md:text-8xl font-bold mb-6 md:mb-8 tracking-tight">
							<span className="bg-clip-text  bg-gradient-to-b text-black dark:text-white">{t("headline1")}</span>
							<br />
							<span className={cn("bg-clip-text  text-black dark:text-white ", pacifico.className)}>{t("headline2")}</span>
						</h1>
					</motion.div>
				</div>
			</div>
