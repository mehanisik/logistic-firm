...
export function ContactForm({ className }: { className?: string }) {
	...
	const validate = () => {
		...
		else if (fields.name.trim().length < 2) newErrors.name = t("validation.nameMin")

		if (!fields.email.trim()) newErrors.email = t("validation.emailRequired")
		else if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(fields.email.trim())) newErrors.email = t("validation.emailInvalid")

		if (!fields.message.trim()) newErrors.message = t("validation.messageRequired")
		else if (fields.message.trim().length < 10) newErrors.message = t("validation.messageMin")
	...
	return (
		<Card className={className ? className : "w-full p-8 border border-border bg-background/90 shadow-lg"}>
			...
			<CardContent className="p-0">
				<form onSubmit={handleSubmit} className="flex flex-col gap-6" noValidate>
					<div className="flex flex-col gap-2">
						...
						<Input id="name" name="name" value={fields.name} onChange={handleChange} placeholder={t("namePlaceholder")} required />
					...
					<div className="flex flex-col gap-2">
						...
						<Input id="company" name="company" value={fields.company} onChange={handleChange} placeholder={t("companyPlaceholder")} />
					</div>
					<div className="flex flex-col gap-2">
