    ...
    >
      <div className="group relative m-auto max-w-7xl">
        <div className="flex flex-col items-center md:flex-row md:gap-6 lg:gap-8">
          <div className="md:max-w-[280px] lg:max-w-xs text-center md:text-end mb-4 sm:mb-6 md:mb-0 md:border-r md:pr-6 lg:pr-8">
              ...
              <div className="flex shrink-0 justify-around [gap:var(--gap)] animate-marquee flex-row group-hover:[animation-play-state:paused]">
                {[...Array(4)].map((_, setIndex) =>
                  PARTNER_LOGOS.map((logo: LogoInfo, index: number) => (
                    <div
                      className="flex-shrink-0 h-6 sm:h-8 md:h-10 mx-0.5 sm:mx-1"
                      key={`${logo.alt}-${index}`}
                    >
                      <Image
